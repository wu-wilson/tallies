...
/**
 * Build the prefilled `venmo.com` payment URL — a universal/app link that opens the Venmo app when
 * installed and the web page otherwise. Most reliable when navigated via a tapped `<a>`, not JS.
 * @param params - Recipient handle, amount, and memo
 * @returns A `https://venmo.com/...` URL with encoded `txn`/`amount`/`note` params
 */
export function buildVenmoPaymentUrl({ username, amount, memo }: VenmoPaymentParams): string {
  ...
  return `https://venmo.com/${recipient}?txn=pay&amount=${amountParam}&note=${note}`;
}
