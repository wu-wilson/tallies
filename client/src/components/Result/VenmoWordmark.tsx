...
interface VenmoWordmarkProps {
  /** Wordmark width in px; defaults to 52. */
  width?: number;
  /** Wordmark height in px; defaults to 13 (matches the glyph's cropped vertical band at the default width). */
  height?: number;
  /** Sets the fill via a text-color utility — `text-white` on the pay button, `text-venmo` on the field. */
  className?: string;
}

/**
 * The Venmo wordmark (Simple Icons), cropped to the glyph's vertical band. Fills with `currentColor`, so
 * its color comes from the parent's text-color utility rather than a hardcoded value.
 * @param props - Optional dimensions and a `className` carrying the text color
 * @returns Inline SVG wordmark, marked decorative (`aria-hidden`)
 */
export const VenmoWordmark: React.FC<VenmoWordmarkProps> = ({ width = 52, height = 13, className }) => (
  <svg viewBox="0 9 24 6" width={width} height={height} fill="currentColor" className={className} aria-hidden="true">
