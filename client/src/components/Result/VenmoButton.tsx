...
import { openVenmoPayment } from '../../lib/venmo';
...
/**
 * Per-person pay button showing the white Venmo wordmark on the brand blue — opens Venmo with the
 * recipient, amount, and memo prefilled. Lightens on hover, mirroring the app's primary CTA.
 * @param props - Recipient handle, amount, and memo for the payment
 * @returns Full-width Venmo button
 */
export const VenmoButton: React.FC<VenmoButtonProps> = ({ username, amount, memo }) => {
  return (
    <motion.button
      type="button"
      onClick={() => openVenmoPayment({ username, amount, memo })}
      ...
      whileTap={{ scale: 0.98 }}
    >
      {/* Venmo wordmark (Simple Icons), viewBox cropped to the glyph's vertical band */}
      <svg viewBox="0 9 24 6" width="52" height="13" fill="currentColor" aria-hidden="true">
